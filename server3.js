...
app.post('/admin-login', function(req,res){
		...
		(err,result) => {
			if(err) {
				res.status(400)
				res.json({"Message" : "Not_Found_Admin"})
			}
                	else{
	...
	.then( 
		...
		if(err) {
			res.status(400)
			res.json({"Message" : "KeyError"})		
		}
		else {
			if(result == true) {
				var token = jwt.sign({ user_id : 1}, secretKey)
				res.json({'token':token})
			} else {
				res.json({"Message": "Invalid_paasword"})
...
app.get('/movie-url', function(req, res){
	...
	client.query(query, function(err, result){
		if(err){
			console.log(err)
			res.json({"Message" : "Invalid_Language"})
		} else {
			try{
			...
			client.query(query, function(err, result){
				if(err){
					...
					let return_result = {
						...
						"PC_reward_url" : result.rows[0].mo_image_url,
...
app.get('/quiz/:quizNum', function(req, res){
	...
	client.query(quiz_query, function(err, result){
		if(err){
			...
			let query = `select * from answers where language_id=${language_id} and quiz_id=${quiz_id};`
			client.query(query, function(err, result){
				if(err){
					console.log(err)
					res.send({"Message": "Error"})
...
app.post('/quiz', function(req, res){
	...
	let user_id = user.user_id
        ...
        let params = [ user_id, quiz_num, answer ];
        client.query(sql, params, function(err, result){
		if(err) {
			console.log(err)
			res.send(err)
